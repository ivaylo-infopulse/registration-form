...
// recursive function to solve the game
  ...
  },[isReady.isComplate, isSolved])

  const getTimeScore=(theTime)=>{
    const existingData = JSON.parse(localStorage.getItem('userData'))
    const userId = existingData.findIndex((data) => data?.name === user?.name)
    
    // check for if user have time score and update for the best scored time
    if(existingData[userId].timeScore > theTime && theTime || !existingData[userId].timeScore){
      ...
      <table>
        <tbody className="sudoku-board">
          ...
          {[0, 1, 2, 3, 4, 5, 6, 7, 8].map((row, rowIndex) => (
            <tr key={rowIndex} className={(row + 1)%3 === 0? 'bRow row': 'row'}>
              {[0, 1, 2, 3, 4, 5, 6, 7, 8].map((col, colIndex) => (
                <td key={colIndex} className={(col + 1)%3 === 0? 'rCell cell': 'cell'}>
